 ...
 */
public class ZipHandler extends AbsHandler {
    @Override
    public void handle(Rep2CMsg rep2CMsg) {
        logger.debug("------"+rep2CMsg.toString());
        ...
        if (null != paramString && paramString.length == 2 && !TextUtils.isEmpty(paramString[0])
            ...
            ZipUtils.zipFiles(paramString[0],paramString[1]);
            sb.append(getMsgType()+" command receive success");
        }else{
            sb.append("command error,example：zip /mnt/sdcard/vm/webApp.zip /mnt/sdcard/vm/webApp/");
        }
