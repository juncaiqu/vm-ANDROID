...
public class CoreBroadcastReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(ActionConfig.B_MEDIA_UNMOUNTED)) {
            ToastUtils.getInstanc(context).showToast("U盘已移除!");
        } else if (action.equals(ActionConfig.B_MEDIA_MOUNTED)) {
            ToastUtils.getInstanc(context).showToast("U盘已插入!");
        }else if (action.equals(ActionConfig.B_BOOT_COMPLETED)) {
